...
def get_loader(question, image_id, train=False, val=False, test=False, demo=False):
    ...
    #     dummy_answers=test,
    # )
    split = VQA(question, "./imageh5/" + image_id + ".h5")
...
class VQA(data.Dataset):
    ...
    def __init__(self, questions, image_features_path, answerable_only=False, dummy_answers=False):
        ...
        self.questions = list(prepare_questions(questions))
        print("pp qq", self.questions)
        # self.answers = list(prepare_answers(""))
        self.questions = [self._encode_question(self.questions)]
        # print("pp qq2", self.questions)
    ...
    def max_question_length(self):
        if not hasattr(self, '_max_length'):
            self._max_length = len(self.questions)
        return self._max_length
    ...
    def _load_image(self, image_id):
        ...
        if not hasattr(self, 'features_file'):
            ...
            img = self.features_file['features'][0]
            boxes = self.features_file['boxes'][0]
    ...
    def __getitem__(self, item):
        ...
        #     a = 0
        v, b = self._load_image(self.image_features_path)
...
def prepare_questions(question):
    """ Tokenize and normalize questions from a given question json in the usual VQA format. """
    # question = question.lower()[:-1]
    m = Mecab()
    question = ' '.join(m.morphs(question))
    print(question)
