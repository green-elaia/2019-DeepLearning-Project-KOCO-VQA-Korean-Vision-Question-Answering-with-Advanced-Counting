...
class Net(nn.Module):
    ...
    def __init__(self, embedding_tokens):
        ...
        self.text = TextProcessor(
            ...
            embedding_features=300,#300,
            lstm_features=question_features,
            drop=0.5,
        )
        self.attention = Attention(
            v_features=vision_features,#2048
            q_features=question_features,#1024
            mid_features=512,
            # mid_features=256,
            glimpses=glimpses,
            drop=0.5,
...
class Fusion(nn.Module):
    ...
    def forward(self, x, y):
        ...
        #print("fusion", (- (x - y)**2 + F.relu(x + y)).shape)
...
class TextProcessor(nn.Module):
    ...
    def forward(self, q, q_len): # q 집합
        embedded = self.embedding(q)
        #print("emb", embedded.shape) #q=41
        tanhed = self.tanh(self.drop(embedded))
        #print("tan", tanhed.shape)
        packed = pack_padded_sequence(tanhed, q_len, batch_first=True)
        self.lstm.flatten_parameters() # GRU 메모리 성능을 위해 추가함 ]
        _, h = self.lstm(packed)
        #print("last", h.squeeze(0).shape)
        return h.squeeze(0)
...
class Attention(nn.Module):
    def __init__(self, v_features, q_features, mid_features, glimpses, drop=0.0):
        ...
        self.v_conv = nn.Conv2d(v_features, mid_features, 1, bias=False) #2048,512  # let self.lin take care of bias
        self.q_lin = nn.Linear(q_features, mid_features) #1024,512
        self.x_conv = nn.Conv2d(mid_features, glimpses, 1) #512,2,1 #1024-1
