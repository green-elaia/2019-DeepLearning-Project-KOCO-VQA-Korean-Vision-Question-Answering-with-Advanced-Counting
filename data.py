...
print("FIN get bert model")
model.eval()


def get_loader(train=False, val=False, test=False):
    #print("testtestestestsetsts")
...
class VQA(data.Dataset):
    ...
    def _find_answerable(self, count=True):#False):
    ...
    def _encode_question(self, question):
        ...
        return sentence_embedding, len(sentence_embedding)
        # vec = []
        # for i, token in enumerate(question):
        #     # print(i)
        #     # print(token)
    ...
    def _encode_answers(self, answers): #  262148\n'
    ...
    def _load_image(self, image_id):
        ...
        # image_id = re.sub('[^0-9a-zA-Zㄱ-힗]', '', image_id)
        # print('image_id : ', image_id, "  ", 'type : ', type(image_id))
        # image_id = int(str(image_id).rstrip('\n'))
        # image_id = int(re.sub('[^0-9]', '', str(image_id)))
        ...
        try:
            ...
            img = self.features_file['features'][index]
            boxes = self.features_file['boxes'][index]
            # print('1st : ', torch.from_numpy(img).unsqueeze(1).shape)
            # print('2nd : ', torch.from_numpy(boxes).shape)
            return torch.from_numpy(img).unsqueeze(1), torch.from_numpy(boxes)
        except:
            print('load_image error occur......!!')
            print('image_id : ', image_id)
            img = torch.randn(2048, 1,  100, dtype=torch.int)
            boxes = torch.randn(4, 100, dtype=torch.int)
            return img, boxes
    ...
    def __getitem__(self, item):
        ...
        # if self.answerable_only:
        #     item2 = self.answerable[idx]
        # item = {}
        # item['q'], item['q_length'] = self.questions[idx]
        # if not self.dummy_answers:
        #     item['a'] = self.answers[idx]
        # else:
        #     # just return a dummy answer, it's not going to be used anyway
        #     item['a'] = 0
        # image_id = self.coco_ids[idx]
        # item['v'], item['b'] = self._load_image(image_id)
        # # item['idx'] = idx
        # return item, item2
...
def prepare_questions(questions_json):
    ...
    #print("data func" , questions[0:10])
